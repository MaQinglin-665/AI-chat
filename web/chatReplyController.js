(function (root) {
  ...
  function resolveChatPayloadBuilder() {
    const api = root.TaffyModules?.chatPayloadBuilder;
    if (api && typeof api.buildAssistantRequestPayload === "function") {
      return api;
    }
    if (typeof module !== "undefined" && module.exports && typeof require === "function") {
      try {
        return require("./chatPayloadBuilder.js");
      } catch (_) {
        // Browser builds load this through index.html before chatReplyController.js.
      }
    }
    return {};
  }

  const CHAT_PAYLOAD_BUILDER = resolveChatPayloadBuilder();
  ...
  function createController(deps = {}) {
    ...
    function cleanConversationContextText(value, maxLen = 360) {
      return CHAT_PAYLOAD_BUILDER.cleanConversationContextText(value, maxLen);
    }

    function classifyBargeInReplyPolicy(value) {
      return CHAT_PAYLOAD_BUILDER.classifyBargeInReplyPolicy(value);
    }
    ...
    function normalizeInputModality(value, fallback = "text") {
      return CHAT_PAYLOAD_BUILDER.normalizeInputModality(value, fallback);
    }

    function normalizeAsrConversationContext(value = null) {
      return CHAT_PAYLOAD_BUILDER.normalizeAsrConversationContext(value);
    }
    ...
    async function requestAssistantReply(text, opts = {}) {
      ...
      try {
        ...
        const payloadBuild = CHAT_PAYLOAD_BUILDER.buildAssistantRequestPayload({
          message,
          ...
          clientSendTsMs: chatPerfStartWallMs,
          interruptionContext,
          asrContext,
          autoKind: opts.autoKind,
          autoThoughtBurst: opts.autoThoughtBurst,
          imageDataUrl,
          characterExperienceProfile: getCharacterExperienceRequestProfile()
        });
        const payload = payloadBuild.payload;
        if (payloadBuild.replyPolicy) {
          if (state.turnManagerLastDecision && typeof state.turnManagerLastDecision === "object" && !Array.isArray(state.turnManagerLastDecision)) {
            state.turnManagerLastDecision.barge_in_kind = payloadBuild.replyPolicy.kind;
            state.turnManagerLastDecision.next_reply_move = payloadBuild.replyPolicy.reply_move;
            state.turnManagerLastDecision.next_reply_goal = payloadBuild.replyPolicy.goal;
          }
        }
