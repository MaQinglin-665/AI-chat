(function (root) {
  ...
  function createController(deps = {}) {
    ...
    function buildPlannedMotionGroups(style, intent, mood, source) {
      ...
      const semantic = findSemanticMotionGroups(getSemanticMotionTags(intent, mood, style, source), 10);
    ...
    function buildActionPlan(intent, context = {}) {
      ...
      if (intent === "tap") {
        ...
        if (Math.random() <= preset.tapChance) {
          steps.push({ mood: tapMood, source: "tap", groups: buildPlannedMotionGroups(style, "tap", tapMood, "tap"), priority: 3, force: true, cooldownMs: 620, allowFallback: false });
        }
        if (comboEnabled && Math.random() < preset.comboChance) {
          const followMood = ["happy", "surprised", "idle"][Math.floor(Math.random() * 3)] || "idle";
          steps.push({ mood: followMood, source: "talk", groups: buildPlannedMotionGroups(style, "talk", followMood, "talk"), priority: 2, force: true, cooldownMs: 520, delayMs: 160, allowFallback: false });
        }
        return steps;
      }
      ...
      if (intent === "listen") {
        ...
        if (Math.random() < (Number(preset.listenChance) || 0.78)) {
          steps.push({ mood: listenMood, source: "talk", groups: buildPlannedMotionGroups(style, "listen", listenMood, "talk"), priority: 2, force: false, cooldownMs: 900, allowFallback: false });
        }
        if (comboEnabled && Math.random() < (Number(preset.comboChance) || 0.4) * 0.28) {
          steps.push({ mood: listenMood, source: "idle", groups: buildPlannedMotionGroups(style, "thinking", listenMood, "idle"), priority: 1, force: false, cooldownMs: 520, delayMs: 120, allowFallback: false });
        }
        return steps;
      }

      if (intent === "thinking") {
        steps.push({ mood: "idle", source: "idle", groups: buildPlannedMotionGroups(style, "thinking", "idle", "idle"), priority: 2, force: false, cooldownMs: 1200, allowFallback: false });
        if (comboEnabled && Math.random() < (Number(preset.thinkingComboChance) || 0.4)) {
          steps.push({ mood: "idle", source: "idle", groups: buildPlannedMotionGroups(style, "idle", "idle", "idle"), priority: 1, force: false, cooldownMs: 640, delayMs: 180, allowFallback: false });
        }
        return steps;
      }
      ...
      if (intent === "talk") {
        ...
        steps.push({ mood: talkMood, source: style === "playful" ? "tap" : "talk", groups: buildPlannedMotionGroups(style, "talk", talkMood, style === "playful" ? "tap" : "talk"), priority: 2, force: true, cooldownMs: Math.max(520, Math.round(780 - emphasis * 140)), allowFallback: false });
        ...
        if (comboEnabled && (emphasis > 0.82 || Math.random() < comboChance)) {
          ...
          steps.push({ mood: accentMood, source: "talk", groups: buildPlannedMotionGroups(style, "reply", accentMood, "talk"), priority: 2, force: true, cooldownMs: Math.max(460, Math.round(540 - emphasis * 60)), delayMs: Math.max(130, Math.round((180 + beats * 92) - emphasis * 38)), allowFallback: false });
        }
        return steps;
      }

      if (intent === "reply") {
        steps.push({ mood, source: style === "playful" ? "tap" : "emotion", groups: buildPlannedMotionGroups(style, "reply", mood, style === "playful" ? "tap" : "emotion"), priority: 3, force: true, cooldownMs: 860, allowFallback: true });
        if (comboEnabled && Math.random() < (preset.comboChance * 0.8)) {
          const follow = style === "comfort" ? "idle" : mood;
          steps.push({ mood: follow, source: "talk", groups: buildPlannedMotionGroups(style, "talk", follow, "talk"), priority: 2, force: true, cooldownMs: 560, delayMs: 170, allowFallback: false });
        }
        if (comboEnabled && Math.random() < (Number(preset.replyAccentChance) || 0.42)) {
          const tailMood = style === "playful" ? "happy" : "idle";
          steps.push({ mood: tailMood, source: "idle", groups: buildPlannedMotionGroups(style, "idle", tailMood, "idle"), priority: 1, force: false, cooldownMs: 480, delayMs: 280, allowFallback: false });
        }
        return steps;
      }

      steps.push({ mood: idleMood, source: "idle", groups: buildPlannedMotionGroups(style, idleIntent, idleMood, "idle"), priority: 2, force: false, cooldownMs: 1000, allowFallback: false });
      if (comboEnabled && Math.random() < (Number(preset.idleComboChance) || 0.3)) {
        steps.push({ mood: "idle", source: "idle", groups: buildPlannedMotionGroups(style, "thinking", "idle", "idle"), priority: 1, force: false, cooldownMs: 520, delayMs: 220, allowFallback: false });
      }
      return steps;
    }
    ...
    async function runActionQueue() {
      ...
      try {
        while (state.actionQueue.length > 0) {
          ...
          await call(deps.playEmotion, step.mood || "idle", {
    ...
    return { uniqueMotionGroups, normalizeMotionGroupKey, findSemanticMotionGroups, getSemanticMotionTags, getStyleMotionGroups, buildPlannedMotionGroups, shouldThrottleActionIntent, clearThinkingMotionTimer, resetActionSystem, buildActionPlan, isTalkLikeActionStep, hasPendingTalkLikeAction, shouldSkipActionStepForSpeech, runActionQueue, enqueueActionIntent };
  }

  const api = { STYLE_MOTION_BLUEPRINT, MOTION_SEMANTIC_TOKENS, uniqueMotionGroups, normalizeMotionGroupKey, createController };
