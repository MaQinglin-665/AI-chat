...

{
  ...
  assert.strictEqual(interjection.director.thought_type, "tiny_rant", "stage interjections should become a thought burst, not a forced one-liner");
...
assert.ok(
  tuningSource.includes("function buildFeedbackMessage")
    ...
    && autoChatControllerSource.includes("thought_type")
...
assert.ok(
  /progressTimer\s*=\s*window\.setInterval\(async \(\) => \{[\s\S]*?performance\.now\(\) - lastProgressAt < 2800[\s\S]*?playAudioByContext\(blob,\s*contextPlaybackDebugContext(?:,\s*notifyPlaybackStart)?\)/.test(ttsPlaybackControllerSource),
